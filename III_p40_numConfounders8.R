# Scenario III
# randomized studies (no need to estiamte e_k)
...
######################################################################################
sim_each <- function(n, p, K, data, covariate_cols, X_int_basis, cols_re_tau, cols_re_mu0, cols_re_a, beta_tau, beta_mu0, beta_a, beta_mu0_k, beta_a_k, beta_tau_k, p_mat, perturb_tau, perturb_mu0, perturb_a, k_folds, alpha, prop_overlap){
  check <- rep(0, K)
  while(any(check == 0)){
    # Test set indices
  ...
  m_mat <- sapply(1:K, function(k){
    (X_int %*% beta_mu0_k[[k]] + 0.5 * as.matrix(X_int_basis) %*% beta_tau_k[[k]]) * p_mat[, k]
  })
  ...
  X_res_mat_with_int <- lapply(1:K, function(k){
    sweep(X_int_basis[-ind_test, ], 1, (data[-ind_test, ]$A - 0.5) * p_mat[-ind_test, k], "*")
  })
  ...
  X_res_mat_merge <- lapply(1:K, function(k){
    sweep(X_int_basis[-ind_test, ], 1, (data[-ind_test, ]$A - 0.5) * p_pred_mat_train[, k], "*")
  })
...
sim_multiple <- function(edat_all, n, p, K, covariate_cols, perturb_tau, perturb_mu0, perturb_a, k_folds, alpha, prop_overlap, num_confounders, nreps){
  ...
  beta_tau <- rnorm(p + ncol(spline.design), mean = 2, sd = 1)
  ...
  # Beta_tau_k
  for(k in 1:(K)){
    new_coefs <- rep(0, p + ncol(spline.design))
    new_coefs[coef_ind[[k]]] <- rnorm(length(coef_ind[[k]]), 0, 1)
...
for(o in ind_o){
  for(t in ind_t){
    results[[o]][[t]] = as.data.frame(sim_multiple(edat_all = edat_all, n = n, p = p, K = K, covariate_cols = covariate_cols, 
...
save(results, perturb_tau, perturb_mu0, perturb_a, prop_overlap, num_confounders, K, n, p, file = paste0("Simulation_III_p40_numConfounders8", ".RData"))
