...
######################################################################################
sim_each <- function(n, p, K, data, covariate_cols, cols_re_tau, cols_re_mu0, cols_re_a, beta_tau, beta_mu0, beta_a, beta_mu0_k, beta_a_k, beta_tau_k, p_mat, perturb_tau, perturb_mu0, perturb_a, k_folds, alpha){
  check <- rep(0, K)
  while(any(check == 0)){
    # Test set indices
  ...
  m_mat <- sapply(1:K, function(k){
    (X_int %*% beta_mu0_k[[k]] + expit(X_int %*% beta_a_k[[k]]) * X_int %*% beta_tau_k[[k]]) * p_mat[, k]
  })
  ...
  X_res_mat_with_int <- lapply(1:K, function(k){
    sweep(X_int[-ind_test, ], 1, (data[-ind_test, ]$A - expit(X_int[-ind_test, ] %*% beta_a_k[[k]])) * p_mat[-ind_test, k], "*")
  })
  ...
  X_res_mat_merge <- lapply(1:K, function(k){
    sweep(X_int[-ind_test, ], 1, (data[-ind_test, ]$A - a_estimate_merge) * p_pred_mat_train[, k], "*")
  })
  ...
  X_res_mat_avg <- lapply(1:K, function(k){
    sweep(X_int[-ind_test, ], 1, (data[-ind_test, ]$A - predict(a_hat_k[[k]], newx = as.matrix(data[-ind_test, covariate_cols]), s = a_hat_k_lambda_min[k], type = "response")) * p_pred_mat_train[, k], "*")
  })
...
sim_multiple <- function(n, p, K, covariate_cols, perturb_tau, perturb_mu0, perturb_a, k_folds, alpha, numRE, nreps){
  ...
  # For selecting study-specific RE
  coef_ind[[1]] <- c()
...
for(a in ind_a){
  ...
  for(m in ind_mu0){
    ...
    for(t in ind_t){
      results[[a]][[m]][[t]] <- as.data.frame(sim_multiple(n = n, p = p, K = K, covariate_cols = covariate_cols, 
...
save(results, perturb_tau, perturb_mu0, perturb_a, numRE, K, n, p, file = paste0("Scenario_A", ".RData"))
