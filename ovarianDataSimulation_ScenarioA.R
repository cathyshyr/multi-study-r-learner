...
######################################################################################
sim_each <- function(n, p, K, data, covariate_cols, cols_re_tau, cols_re_mu0, cols_re_a, beta_tau, beta_mu0, beta_a, p_mat, perturb_tau, perturb_mu0, perturb_a, k_folds, alpha, iteration){
  check <- rep(0, K - 1)
  while(any(check == 0)){
    # Test set indices 
    ...
    beta_tau_k <- beta_a_k <- beta_mu0_k <- vector("list", length = K - 1)
  ...
  Y_mat <- sapply(1:(K - 1), function(k){
    (X_int_train %*% beta_mu0_k[[k]] + data_train$A * X_int_train %*% beta_tau_k[[k]]) * p_mat[-ind_test, k]
  })
  data_train$Y <- apply(Y_mat, 1, sum) + rnorm(nrow(data_train), 0, 1)
  
  # Oracle m
  m_mat <- sapply(1:(K - 1), function(k){
    (X_int_train %*% beta_mu0_k[[k]] + expit(X_int_train %*% beta_a_k[[k]]) * X_int_train %*% beta_tau_k[[k]]) * p_mat[-ind_test, k]
  })
  ...
  # Calculate additional metrics for revision
  B <- 500
  ...
  for (b in 1:B) {
    X_k_b <- X_k_b_int <- Y_k_b <- W_k_b <- data_train_k_b <- p_mat_k_b <- true_e_k_b <- true_m_k_b <- vector("list", length = K - 1)
    ...
    for(k in 1:(K - 1)){
      ...
      data_train_k_b[[k]] <- data_train[idx, ]
    }
    # Combine to form training dataset
    X_b <- do.call("rbind", X_k_b)
    Y_b <- unlist(Y_k_b)
    W_b <- unlist(W_k_b)
    data_train_b <- do.call("rbind", data_train_k_b)
    X_int_b <- do.call("rbind", X_k_b_int)
    p_mat_b <- do.call("rbind", p_mat_k_b)
    m_mat_b <- sapply(1:(K - 1), function(k){
      (X_int_b %*% beta_mu0_k[[k]] + expit(X_int_b %*% beta_a_k[[k]]) * X_int_b %*% beta_tau_k[[k]]) * p_mat_b[, k]
    })
    true_m_b <- apply(m_mat_b, 1, sum)
    
    # Estimate m (merge)
    m_estimate_merge_b <- rlearner_m_hat_merge(data = data_train_b, K = K - 1, p = p, covariate_cols = covariate_cols, k_folds = k_folds, alpha = alpha)
    
    # Estimate a 
    a_estimate_merge_b <- rlearner_a_hat_merge(data = data_train_b, K = K - 1, p = p, covariate_cols = covariate_cols, k_folds = k_folds, alpha = alpha)
    
    ################################################
    #########   MS R-learner Merge Both    #########
    ################################################
    # Estimate ascertainment probability on training data
    p_mod_b <- glmnet::cv.glmnet(x = as.matrix(data_train_b[, covariate_cols]),
                                 y = as.matrix(data_train_b$S),
                                 alpha = alpha,
                                 lambda = NULL,
                                 standardize = TRUE,
                                 keep = TRUE,
                                 family = "multinomial")
    p_hat_lambda_min_b = p_mod_b$lambda[which.min(p_mod_b$cvm[!is.na(colSums(p_mod_b$fit.preval))])]
    p_pred_mat_train_b <- as.data.frame(predict(p_mod_b, newx = as.matrix(data_train_b[, covariate_cols]), s = p_hat_lambda_min_b, type = "response"))
    p_pred_mat_test_b <- as.data.frame(predict(p_mod_b, newx = as.matrix(data[ind_test, covariate_cols]), s = p_hat_lambda_min_b, type = "response"))
    
    X_res_mat_merge_b <- lapply(1:(K - 1), function(k){
      sweep(X_int_b, 1, (data_train_b$A - a_estimate_merge_b) * p_pred_mat_train_b[, k], "*")
    })
    ...
    ################################################
    m_estimate_study_specific_b <- rlearner_m_hat_stack(data = data_train_b, K = K - 1, p = p, covariate_cols = covariate_cols, k_folds = k_folds, alpha = alpha, p_mat = p_pred_mat_train_b)
    ...
    for(k in 1:(K - 1)){
      a_hat_k_b[[k]] <- glmnet::cv.glmnet(x = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]),
      ...
      m_hat_k_b[[k]] <- glmnet::cv.glmnet(x = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]),
    ...
    X_res_mat_avg_b <- lapply(1:(K - 1), function(k){
      sweep(X_int_b, 1, (data_train_b$A - predict(a_hat_k_b[[k]], newx = as.matrix(data_train_b[, covariate_cols]), s = a_hat_k_lambda_min_b[k], type = "response")) * p_pred_mat_train_b[, k], "*")
    })
    ...
    tau_boot_mat_ms_rlearner_mMerge_aSS[b, ] <- ms_rlearner(X = X_res_mat_avg_all_b, Y = Y_res_merge_estimate_b, alpha = alpha, K = K - 1, p = p, p_mat = p_pred_mat_test_b,
                                                            newX = X_int[ind_test, ])
    
    # Avg m, Study-specific A
    Y_res_avg_estimate_b <- data_train_b$Y - m_estimate_avg_b
    tau_boot_mat_ms_rlearner_mAvg_aSS[b, ] <- ms_rlearner(X = X_res_mat_avg_all_b, Y = Y_res_avg_estimate_b, alpha = alpha, K = K - 1, p = p, p_mat = p_pred_mat_test_b,
                                                          newX = X_int[ind_test, ])
    
    # Stack m, Study-specific A
    Y_res_stack_estimate_b <- data_train_b$Y - m_estimate_stack_b
    tau_boot_mat_ms_rlearner_mStack_aSS[b, ] <- ms_rlearner(X = X_res_mat_avg_all_b, Y = Y_res_stack_estimate_b, alpha = alpha, K = K - 1, p = p, p_mat = p_pred_mat_test_b,
                                                            newX = X_int[ind_test, ])
    
    # Study-specific m, Study-specific A
    Y_res_ss_estimate_b <- data_train_b$Y - m_estimate_ss_b
    tau_boot_mat_ms_rlearner_mSS_aSS[b, ] <- ms_rlearner(X = X_res_mat_avg_all_b, Y = Y_res_ss_estimate_b, alpha = alpha, K = K - 1, p = p, p_mat = p_pred_mat_test_b,
                                                         newX = X_int[ind_test, ])
    ...
    for(k in 1:(K - 1)){
      rlearner_oracle_SS_b[[k]] <- rlasso(x = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]),
                                          ...
                                          m_hat = true_m_k_b[[k]],
                                          alpha = alpha)
      rlearner_oracle_SS_pred_b[[k]] <- predict(rlearner_oracle_SS_b[[k]], newx = as.matrix(data[ind_test, covariate_cols]))
      
      rlearner_estimate_SS_b[[k]] <- rlasso(x = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]),
                                            w = as.matrix(data_train_b$A[which(data_train_b$S == k)]),
                                            y = as.matrix(data_train_b$Y[which(data_train_b$S == k)]),
                                            lambda_choice = "lambda.min",
                                            # p_hat = predict(a_hat_k_b[[k]], newx = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]), s = a_hat_k_lambda_min_b[k], type = "response"),
                                            # m_hat = predict(m_hat_k_b[[k]], newx = as.matrix(data_train_b[which(data_train_b$S == k), covariate_cols]), s = m_hat_k_lambda_min_b[k], type = "response"),
                                            alpha = alpha)
      rlearner_estimate_SS_pred_b[[k]] <- predict(rlearner_estimate_SS_b[[k]], newx = as.matrix(data[ind_test, covariate_cols]))
    }
    ...
    X_res_mat_with_int_b <- lapply(1:(K - 1), function(k){
      sweep(X_int_b, 1, (data_train_b$A - expit(X_int_b %*% beta_a_k[[k]])) * p_mat_b[, k], "*")
    })
    ...
    tau_boot_mat_ms_rlearner_oracle[b, ] <- ms_rlearner(X = X_res_all_oracle_b, Y = Y_res_merge_oracle_b, alpha = alpha, K = K - 1, p = p, p_mat = p_mat[ind_test, ],
                                                        newX = X_int[ind_test, ])
