...
data_application <- function(data_all, K, p, alpha, beta_tau1, beta_tau0, perturb_tau, covariate_cols, ind_test, ind0){
  ...
  # Fit integrative R-learner on training set
  # RCT 
  ind1 <- which(data_train$S == 1)
  # Observational
  ind2 <- which(data_train$S == 2)
  X_res_mat_avg1_int <- X_int_train[ind1, ] * (data_train$doxorubicin[ind1] - 0.5)
  X_res_mat_avg2_int <- (X_int_train[ind2, ] + X_int_train[ind2, ]) * (data_train$doxorubicin[ind2] - predict(a_hat_k, newx = data.matrix(data_train[ind2, covariate_cols]), s = a_hat_k_lambda_min, type = "response"))
  X_res_mat_avg_int <- list(X_res_mat_avg1_int, X_res_mat_avg2_int)
  
  X_res_mat_avg_all_int <- as.data.frame(do.call("rbind", X_res_mat_avg_int))
  names(X_res_mat_avg_all_int) <- sapply(1:ncol(X_res_mat_avg_all_int), function(x) paste("X.", x, sep = ""))
  
  Y_res_merge_estimate <- data_train$Y - m_estimate_merge_pred
  
  # Estimate tau
  int_rlearner <- glmnet::cv.glmnet(x = as.matrix(X_res_mat_avg_all_int),
                                   y = as.matrix(Y_res_merge_estimate),
                                   alpha = alpha,
                                   lambda = NULL,
                                   standardize = TRUE,
                                   intercept = FALSE,
                                   standardize.response = TRUE)
  tau_beta_int <- as.vector(t(coef(int_rlearner, s = "lambda.min")[-1]))
  int_rlearner_pred <- 2 * expit(as.matrix(X_int_test) %*% as.matrix(tau_beta_int)) - 1
  
  out <- data.frame(rlearner =  mean((test_true_tau - rlearner_pred)^2),
                    ms_rlearner = mean((test_true_tau - ms_rlearner_pred)^2),
                    int_rlearner = mean((test_true_tau - int_rlearner_pred)^2),
                    test_true_tau = test_true_tau,
                    rlearner_pred = rlearner_pred, 
                    ms_rlearner_pred = ms_rlearner_pred,
                    int_rlearner_pred = int_rlearner_pred)
  return(out)
}
...
save(results, perturb_tau, data_all, K, p, beta_tau1, beta_tau0, file = paste0("data_application_20230610", ".RData"))
